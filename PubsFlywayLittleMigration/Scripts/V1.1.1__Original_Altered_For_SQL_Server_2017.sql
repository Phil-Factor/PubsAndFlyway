SET NOEXEC OFF;
print 'Deleting existing objects and schemas that we''re going to build'
...
IF Object_Id('dbo.titles') IS NOT NULL DROP TABLE dbo.titles;
...
CREATE TABLE publishers
  (
  pub_id CHAR(8) NOT NULL CONSTRAINT UPKCL_pubind PRIMARY KEY CLUSTERED
    CONSTRAINT GetPubidright --
  ...
  city NVARCHAR(40) NULL,
  state CHAR(2) NULL,
  country VARCHAR(30) NULL CONSTRAINT SurelyTheUSA DEFAULT ('USA')
  );

GO
/* titles, when published, their price, total sales, year-to-date sales, advance, royalty  and notes */
CREATE TABLE titles
  (
  title_id tid CONSTRAINT UPKCL_titleidind PRIMARY KEY CLUSTERED,
  [type] NVARCHAR(80) NOT NULL constraint df_titletype DEFAULT ('UNDECIDED'),
  title VARCHAR(80) NOT NULL,
  pub_id CHAR(8) NULL CONSTRAINT FK_TitlesPublishers --
                      FOREIGN KEY REFERENCES dbo.publishers (pub_id),
...
CREATE TABLE titleauthor
  (
  au_id id CONSTRAINT FK_TitleauthorAuthors --
           FOREIGN KEY REFERENCES dbo.authors (au_id),
  title_id tid CONSTRAINT FK_TitleauthorTitles --
               FOREIGN KEY REFERENCES dbo.titles (title_id),
  au_ord TINYINT NULL,
  royaltyper INT NULL,
...
GO
/*  record of sales to book stores */
CREATE TABLE sales
  (
  stor_id CHAR(8) NOT NULL CONSTRAINT FK_salesStores --
                           FOREIGN KEY REFERENCES dbo.stores (stor_id),
  ord_num NVARCHAR(40) NOT NULL,
  ord_date DATETIME NOT NULL,
  qty SMALLINT NOT NULL,
  payterms VARCHAR(12) NOT NULL,
  title_id tid CONSTRAINT FK_salesTitles --
               FOREIGN KEY REFERENCES dbo.titles (title_id),
  CONSTRAINT UPKCL_sales PRIMARY KEY CLUSTERED (stor_id, ord_num, title_id)
  );

GO
/* the schedfule of royalties for individual books */
CREATE TABLE roysched
  (
  title_id tid CONSTRAINT FK_RoySchedTitles --
               FOREIGN KEY REFERENCES dbo.titles (title_id),
  lorange INT NULL,
  hirange INT NULL,
  royalty INT NULL
...
CREATE TABLE pub_info
  ...
  pub_id CHAR(8) NOT NULL --
    CONSTRAINT FK_Pub_infoPublishers --
    FOREIGN KEY REFERENCES dbo.publishers (pub_id) CONSTRAINT UPKCL_pubinfo PRIMARY KEY CLUSTERED,
  logo image NULL,
  pr_info text NULL
  );
...
CREATE TABLE employee
  ...
  job_id INT NOT NULL CONSTRAINT LetsMakeItOne DEFAULT 1 --
    CONSTRAINT FK_EmployeeJobs FOREIGN KEY REFERENCES dbo.jobs (job_id),
  job_lvl TINYINT CONSTRAINT DefaultToTen DEFAULT 10,
  pub_id CHAR(8) NOT NULL
    CONSTRAINT HighNumber DEFAULT ('9952') --
    CONSTRAINT FK_EmployeePublishers FOREIGN KEY REFERENCES dbo.publishers
                                       (pub_id),
...
PRINT 'Now at the create trigger section ...';
...
print 'Insert into authors table'
...
print'Now at the inserts to publishers ....';
...
print'Now at the inserts to pub_info ....';
...
print 'Now at the inserts to titles ....'
...
( N'PC9999', 'Net Etiquette', 'popular_comp', '1389',null, null,null,null, 'A must-read for computer conferencing.', N'2020-06-05T12:41:13.163' ), 
...
print'Now at the inserts to titleauthor ....';
...
print'Now at the inserts to stores ....';
...
print'Now at the inserts to sales ....';
...
print'Now at the inserts to roysched ....';
...
print'Now at the inserts to employee ....';
