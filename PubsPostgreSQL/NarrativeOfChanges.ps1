...
$TheArgs+=if ([string]::IsNullOrEmpty($parameters.Username))
    {
    @("-url=jdbc:postgresql://$($parameters.Servername):$($parameters.Port)/$($parameters.Database);integratedSecurity=true")
    }
else
    {
...
@('1.1.1', '1.1.2', '1.1.3', '1.1.4', '1.1.5') | foreach{
    ...
    {   #see what is changed by comparing the models before and after the migration
        $Comparison=Diff-Objects <#get the previous model from file#>`
...
@('1.1.2', '1.1.3', '1.1.4', '1.1.5') | foreach{
	...
	$changes | foreach{
		...
		{
			'->' { "$(if ($current.Target -ne 'PSCustomObject'){"Added '$($current.Target)' to"} else {"Added"}) '$($current.Ref) with version $version'" }
			'<>' { "Changed '$($current.Source)' to '$($current.Target)' at '$($current.Ref) with version $version'" }
			'<-' { "deleted '$($current.Source)' at '$($current.Ref) with version $version'" }
			default { "couldn't determine a change in version $version'" }
		}
	}
}