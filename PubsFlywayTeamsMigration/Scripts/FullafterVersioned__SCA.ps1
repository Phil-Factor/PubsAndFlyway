...
"Getting values from JDBC connection info"
...
<# Now we need to get the credentials, and the current version #>
...
if (-not (Test-Path "$CurrentPathToWorkingFiles\source.*" -PathType Leaf))
{
	...
	"creating the release artefact"
	$iReleaseArtifact = new-DatabaseReleaseArtifact  `
													-Source $SourceConnectionString ` #reference the source by its connection string
	-Target "$CurrentPathToWorkingFiles\source" # it will fill the blank source directory
	#Export a report of what is in the version, including objects and code issues
	"Creating HTML report"
	$iReleaseArtifact.ReportHtml> "$CurrentPathToWorkingFiles\Reports\build.html"
	...
	"Saving release artefact"
	$iReleaseArtifact | Export-DatabaseReleaseArtifact -path "$CurrentPathToWorkingFiles\Artifact\V$($DatabaseDetails.Version)__release.zip"
}