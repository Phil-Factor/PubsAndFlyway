...
if ($username -ne '') #then it is using SQL Server Credentials
{
    ...
    $SQLCmdArgs =@{'Server'=$Server;'Database'=$Database; 'User'=$SqlCredentials.UserName;
        'Password'= $SqlCredentials.GetNetworkCredential().password}
}
else
{
...
if ($VersionNumber -ne $null)
    {
$FlyWayArgs +=
    @("-placeholders.canDoJSON=$(if ($VersionNumber -ge 13) {'true'} else {'false'})",
    "-placeholders.canDoStringAgg=$(if ($VersionNumber -ge 14) {'true'} else {'false'})")
    }
